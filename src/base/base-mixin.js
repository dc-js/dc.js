...
export class BaseMixin {
    constructor () {
        ...
        self._minWidth = 200;
        self._defaultWidthCalc = function (element) {
        ...
        self._minHeight = 200;
        self._defaultHeightCalc = function (element) {
        ...
        self._pvt_title = function (d) {
            return self.keyAccessor()(d) + ': ' + self.valueAccessor()(d);
        };
        ...
        self._filters = [];
        self._filterHandler = function (dimension, filters) {
            if (filters.length === 0) {
         ...
         * @returns {d3.selection}
         */
        self.selectAll = function (sel) {
            return self._root ? self._root.selectAll(sel) : null;
        };
        ...
        self._activateRenderlets = function (event) {
         ...
         * @returns {dc.baseMixin}
         */
        self.redrawGroup = function () {
            if (self._commitHandler) {
                self._commitHandler(false, function (error, result) {
                    if (error) {
                        console.log(error);
                    } else {
        ...
        self._invokeFilteredListener = function (f) {
            if (f !== undefined) {
                self._listeners.call('filtered', self, self, f);
            }
        };

        self._invokeZoomedListener = function () {
            self._listeners.call('zoomed', self, self);
        };

        self._hasFilterHandler = function (filters, filter) {
         ...
         * @returns {Boolean}
         */
        self.hasFilter = function (filter) {
            return self._hasFilterHandler(self._filters, filter);
        };

        self._removeFilterHandler = function (filters, filter) {
            for (let i = 0; i < filters.length; i++) {
        ...
        self._addFilterHandler = function (filters, filter) {
            filters.push(filter);
            return filters;
        };
        ...
        self._resetFilterHandler = function (filters) {
            return [];
        };
         ...
         * @returns {dc.baseMixin}
         **/
        self.replaceFilter = function (filter) {
        ...
        self.legendables = function () {
            // do nothing in base, should be overridden by sub-function
            return [];
        };

        self.legendHighlight = function () {
            // do nothing in base, should be overridden by sub-function
        };

        self.legendReset = function () {
            // do nothing in base, should be overridden by sub-function
        };

        self.legendToggle = function () {
            // do nothing in base, should be overriden by sub-function
        };

        self.isLegendableHidden = function () {
            // do nothing in base, should be overridden by sub-function
            return false;
        };
         ...
         * @returns {dc.baseMixin}
         */
        self.renderlet = logger.deprecate(function (renderletFunction) {
         ...
         * @returns {String}
         */
        self.chartID = function () {
            return self.__dcFlag__;
        };
